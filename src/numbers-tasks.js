...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const differenceX = (x2 - x1) ** 2;
  const differenceY = (y2 - y1) ** 2;
  const amount = differenceX + differenceY;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosineAngle = productOfVectors / (magnitude1 * magnitude2);
...
function getSumOfNumbers(x1, x2, x3) {
  ...
  return Math.round((x1 + x2 + x3) * 10 ** decimalPlaces) / 10 ** decimalPlaces;
}
